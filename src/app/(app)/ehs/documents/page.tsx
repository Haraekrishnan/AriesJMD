  ...
  return (
    <div className="p-5 md:p-8 max-w-[1600px] mx-auto space-y-6 text-slate-900">
      <div className="text-left">
        <h1 className="text-4xl font-semibold text-slate-900 tracking-tight">Safety Library</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {categories.map((cat, i) => (
          <Card key={i} className="bg-white border-slate-200 hover:shadow-md cursor-pointer transition-all duration-300 group rounded-xl">
            <CardHeader className="pb-4">
              <div className={cn("p-4 rounded-2xl w-fit group-hover:scale-110 transition-transform", cat.bg, cat.color)}>
                <cat.icon className="h-7 w-7" />
              </div>
              <CardTitle className="text-lg font-semibold text-slate-900 mt-4 normal-case tracking-tight">{cat.name}</CardTitle>
            </CardHeader>
            <CardContent>
               <p className="text-sm text-slate-400 font-semibold normal-case tracking-normal">{cat.count} Indexed Files</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card className="bg-white border-slate-200 shadow-sm overflow-hidden rounded-xl">
        <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-50/50">
           ...
           <Button variant="outline" className="h-14 px-8 border-slate-200 text-slate-600 rounded-2xl hover:bg-slate-50 font-semibold normal-case text-sm tracking-normal shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-sm normal-case font-semibold text-slate-400 bg-slate-50/80 border-b border-slate-100">
            ...
            <tbody className="divide-y divide-slate-50">
              {[
                ...
                <tr key={i} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-8 py-6 text-slate-500 font-mono text-xs tracking-normal">{doc.ref}</td>
                  <td className="px-8 py-6">
                    <Badge variant="outline" className="text-sm font-semibold border-slate-200 bg-white text-slate-600 px-3 py-0.5">
                      v{doc.ver}
                    </Badge>
                  </td>
                  <td className="px-8 py-6 text-slate-500 text-xs font-bold normal-case">{doc.date}</td>
        ...
        <div className="p-6 bg-slate-50/50 text-center border-t border-slate-100">
           <p className="text-xs text-slate-400 font-semibold normal-case tracking-normal">End of indexed records</p>
        </div>
      </Card>
    </div>
  );
