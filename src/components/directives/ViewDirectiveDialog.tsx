...
import { Send, UserPlus, Layers, Trash2 } from 'lucide-react';
import type { Directive, DirectiveStatus, Role, Comment } from '@/lib/types';
...
const statusOptions: DirectiveStatus[] = ['New', 'Under Review', 'Action Taken', 'Closed'];

const statusVariant: { [key in DirectiveStatus]: 'default' | 'secondary' | 'destructive' | 'success' } = {
  'New': 'default',
...
export default function ViewDirectiveDialog({ isOpen, setIsOpen, directive: initialDirective }: ViewDirectiveDialogProps) {
  ...
  const participants = useMemo(() => {
    const pIds = new Set([directive.creatorId, directive.toUserId, ...(directive.ccUserIds || [])]);
  ...
  const commentsArray = Array.isArray(directive.comments) 
    ? directive.comments 
    : Object.values(directive.comments || {});
      ...
      <DialogContent className="sm:max-w-3xl grid-rows-[auto,1fr,auto] max-h-[90vh]">
        <DialogHeader>
          <div className="flex justify-between items-start">
            <div>
              <DialogTitle>{directive.subject}</DialogTitle>
              <DialogDescription>
                From: {creator?.name} &middot; To: {recipient?.name}
              </DialogDescription>
            </div>
            <Badge variant={statusVariant[directive.status]}>{directive.status}</Badge>
          </div>
        </DialogHeader>
        ...
        <div className="flex-1 grid md:grid-cols-2 gap-8 py-4 overflow-y-auto pr-6 -mr-6">
          <div className="space-y-4">
            ...
            <p className="text-sm p-4 bg-muted rounded-md min-h-[100px] whitespace-pre-wrap">{directive.body}</p>
            ...
            {showForward && (
                <div className="space-y-2 p-4 border rounded-lg bg-muted/50">
                    <h4 className="font-semibold">Forward / CC Users</h4>
          ...
          <div className="flex flex-col gap-4">
             <div className="space-y-3">
                 ...
                 <div className="flex flex-wrap gap-2">
                     ...
                     {canAddUsers && !showForward && (
                        <Button size="sm" variant="outline" onClick={() => setShowForward(true)} disabled={availableUsersForCC.length === 0}>
                            <UserPlus className="mr-2 h-4 w-4"/>Forward / CC
                        </Button>
                    )}
                 </div>
             </div>
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>This will permanently delete this directive and its conversation history for everyone. This action cannot be undone.</AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
