...
type DirectiveFormValues = z.infer<typeof directiveSchema>;
...
export default function NewDirectiveDialog({ isOpen, setIsOpen }: NewDirectiveDialogProps) {
  ...
  const form = useForm<DirectiveFormValues>({
  ...
  const onSubmit = (data: DirectiveFormValues) => {
    ...
    toast({
      ...
      description: 'Your directive has been sent to the designated recipients.',
    });
    setIsOpen(false);
  };
