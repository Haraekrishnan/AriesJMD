...
import { WiredAngleGrinder } from '@/lib/types';
import { Badge } from '../ui/badge';
import { ScrollArea } from '../ui/scroll-area';

interface WiredAngleGrinderTableProps {
  items: WiredAngleGrinder[];
  onEdit: (item: WiredAngleGrinder) => void;
}
...
export default function WiredAngleGrinderTable({ items, onEdit }: WiredAngleGrinderTableProps) {
  const { can, projects, deleteWiredAngleGrinder } = useAppContext();
  const { toast } = useToast();

  const handleDelete = (itemId: string) => {
    deleteWiredAngleGrinder(itemId);
    toast({
      variant: 'destructive',
      title: 'Item Deleted',
  ...
  if (items.length === 0) {
    return <p className="text-muted-foreground text-center py-8">No wired angle grinders found.</p>;
  }
