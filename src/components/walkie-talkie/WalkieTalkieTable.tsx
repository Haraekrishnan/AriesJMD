...
import { WalkieTalkie } from '@/lib/types';
import { Badge } from '../ui/badge';
import { ScrollArea } from '../ui/scroll-area';

interface WalkieTalkieTableProps {
  items: WalkieTalkie[];
  onEdit: (item: WalkieTalkie) => void;
}
...
export default function WalkieTalkieTable({ items, onEdit }: WalkieTalkieTableProps) {
  const { can, projects, deleteWalkieTalkie } = useAppContext();
  const { toast } = useToast();

  const handleDelete = (itemId: string) => {
    deleteWalkieTalkie(itemId);
    toast({
      variant: 'destructive',
      title: 'Item Deleted',
  ...
  if (items.length === 0) {
    return <p className="text-muted-foreground text-center py-8">No walkie talkies found.</p>;
  }
