...
export default function EmployeePerformanceChart() {
  ...
  const chartData = useMemo(() => {
    return users.map(user => {
      ...
      const done = userTasks.filter(t => t.status === 'Done' || t.status === 'Completed').length;
      const inProgress = userTasks.filter(t => t.status === 'In Progress').length;
      const inReview = userTasks.filter(t => t.status === 'In Review' || t.status === 'Pending Approval').length;
