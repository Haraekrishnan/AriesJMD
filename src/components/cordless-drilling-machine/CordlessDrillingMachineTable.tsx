...
import { CordlessDrillingMachine } from '@/lib/types';
import { Badge } from '../ui/badge';
import { ScrollArea } from '../ui/scroll-area';

interface CordlessDrillingMachineTableProps {
  items: CordlessDrillingMachine[];
  onEdit: (item: CordlessDrillingMachine) => void;
}
...
export default function CordlessDrillingMachineTable({ items, onEdit }: CordlessDrillingMachineTableProps) {
  const { can, projects, deleteCordlessDrillingMachine } = useAppContext();
  const { toast } = useToast();

  const handleDelete = (itemId: string) => {
    deleteCordlessDrillingMachine(itemId);
    toast({
      variant: 'destructive',
      title: 'Item Deleted',
  ...
  if (items.length === 0) {
    return <p className="text-muted-foreground text-center py-8">No cordless drilling machines found.</p>;
  }
