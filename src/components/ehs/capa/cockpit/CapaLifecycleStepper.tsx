...
export default function CapaLifecycleStepper({ observation, viewingStage, onStageSelect }: Props) {
    ...
    return (
        <div className="flex items-center w-full gap-12 text-left">
            {/* Progress Display */}
            <div className="flex flex-col shrink-0">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] leading-none mb-2">Overall Progress</span>
                <div className="flex items-center gap-4">
                    <span className="text-2xl font-black text-blue-600 tracking-tighter leading-none">{stats.percentage}%</span>
                    <div className="w-32">
                        <Progress value={stats.percentage} className="h-1 bg-slate-100 rounded-full" />
                    </div>
                </div>
            </div>
                        ...
                        <React.Fragment key={stage}>
                            <div 
                                className={cn(
                                    ...
                                    isViewing ? "scale-105" : "hover:opacity-80",
                                    isFuture && "opacity-30 grayscale pointer-events-none"
                                )}
                                onClick={() => onStageSelect(stage)}
                            >
                                <div className={cn(
                                    "h-9 w-9 rounded-full border-2 flex items-center justify-center transition-all text-[11px] font-black shadow-sm relative",
                                    isCompleted ? "bg-emerald-500 border-emerald-500 text-white" :
                                    isViewing ? "bg-blue-600 border-blue-600 text-white shadow-md shadow-blue-500/30" :
                                    "bg-white border-slate-200 text-slate-400"
                                )}>
                                    {isCompleted ? <Check className="h-4 w-4 stroke-[4]" /> : <span>{i + 1}</span>}
                                    {isCurrent && !isCompleted && (
                                        <div className="absolute inset-0 rounded-full border-2 border-blue-600 animate-ping opacity-10" />
                                    )}
                                </div>
                                <p className={cn(
