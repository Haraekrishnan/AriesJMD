    ...
    return (
        <div className="flex flex-col h-full bg-white divide-y-2 divide-slate-50 text-left overflow-hidden">
            ...
            <div className="p-6 space-y-6 bg-slate-50/30">
                ...
                <div className="flex items-center gap-4 p-4 rounded-xl bg-white border-2 shadow-sm border-emerald-100">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Activities within timeframe</span>
                    </div>
                </div>
            ...
            <div className="flex-1 p-6 flex flex-col min-h-0 bg-slate-50/20">
                ...
                <ScrollArea className="flex-1">
                    <div className="space-y-6 pb-6 pr-4">
                        <div className="p-4 bg-white border-2 rounded-xl shadow-sm border-slate-100">
                            <p className="text-[9px] font-black uppercase text-slate-400 mb-2 tracking-widest">SYSTEM INITIALIZATION</p>
                            <p className="text-[11px] font-bold text-slate-600 leading-relaxed uppercase tracking-tight">Lifecycle tracking activated for discovery phase.</p>
                        </div>
                    </div>
                </ScrollArea>
            </div>
...
function MetaRow({ label, value, isRisk }: { label: string, value?: string, isRisk?: boolean }) {
    return (
        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
            ...
            {isRisk ? (
                <Badge className="bg-orange-500 text-white border-none font-black px-2 h-5 text-[8px] rounded shadow-sm">{value?.toUpperCase() || 'MEDIUM'}</Badge>
            ) : (
