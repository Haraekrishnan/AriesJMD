        ...
        <div className="space-y-8 text-left">
            ...
            <Dialog open={!!viewingAttachmentUrl} onOpenChange={() => { setViewingAttachmentUrl(null); setZoom(1); setTranslate({x: 0, y: 0}); setNumPages(null); setPageNumber(1); }}>
                <DialogContent className="max-w-[95vw] md:max-w-7xl w-full h-auto max-h-[90vh] flex flex-col p-0 overflow-hidden bg-black border border-white/10 shadow-2xl">
                    <DialogTitle className="px-6 py-4 text-white text-sm font-black uppercase tracking-widest bg-slate-900/50">Evidence Registry Viewer</DialogTitle>
                        ...
                        <div className="flex items-center gap-8">
                            <div className="text-right">
                                ...
                                <div className="mt-1 flex items-center gap-2 justify-end">
                                    <p className="text-[10px] font-extrabold uppercase text-[#102A43]">{users.find(u => u.id === observation.reporterId)?.name || 'Unknown'}</p>
                                    <Avatar className="h-6 w-6 border">
                                        <AvatarImage src={users.find(u => u.id === observation.reporterId)?.avatar} />
                                        <AvatarFallback className="text-[8px]">{users.find(u => u.id === observation.reporterId)?.name?.[0]}</AvatarFallback>
                                    </Avatar>
                                </div>
                            </div>
