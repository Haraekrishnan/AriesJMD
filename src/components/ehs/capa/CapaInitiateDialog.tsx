...
export default function CapaInitiateDialog({
  isOpen,
  onOpenChange,
}: {
  isOpen: boolean;
          ...
          if (blob) {
            ...
            reader.onload = (event) => {
              ...
              toast({
                ...
                description: 'Photo attached to narrative finding.',
              });
            };
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    setHandoffError('');
    try {
      const target = validateHandoff(handoff, users);
      await addObservation({ ...data, reporterId: user.id }, target);
      onOpenChange(false);
      setPastedImage(null);
      form.reset();
    ...
    >
      <DialogContent className="sm:max-w-3xl bg-white p-0 overflow-hidden border-none shadow-2xl rounded-2xl">
        <DialogHeader className="p-8 pb-4">
          <DialogTitle className="text-2xl font-semibold text-[#0F172A] normal-case tracking-tight">
            New safety observation
          </DialogTitle>
          <DialogDescription className="text-xs font-bold text-slate-400 normal-case tracking-normal">
            PHASE 01: INITIAL DISCOVERY & CAPTURE
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="p-8 pt-2 space-y-8">
            ...
            <div className="flex items-start gap-4 p-5 bg-[#EFF6FF] border border-[#DBEAFE] rounded-xl animate-in fade-in duration-500">
              ...
              <p className="text-[13px] font-medium text-[#1E40AF] leading-relaxed">
                Report any unsafe act, unsafe condition, near miss or positive
                safety observation. Your input helps us maintain a safe and
                healthy workplace.
              </p>
            </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <fieldset disabled={form.formState.isSubmitting}>
                <CapaHandoffFields
                  title="Next phase: Investigation"
                  value={handoff}
                  onChange={setHandoff}
                />
              </fieldset>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <div className="h-6 w-6 rounded-full bg-[#2563EB] text-white flex items-center justify-center text-sm font-semibold">
                      1
                    </div>
                    <Label className="text-sm font-semibold normal-case tracking-normal text-[#0F172A]">
                      Safety Finding Narrative{' '}
                ...
                {pastedImage && (
                  <div className="flex items-center gap-3 p-3 bg-emerald-50 border border-emerald-100 rounded-lg animate-in zoom-in-95">
                    ...
                    <p className="flex-1 text-sm font-semibold text-emerald-700 normal-case tracking-normal">
                      EHS DISCOVERY IMAGE ATTACHED
                    </p>
                    <Button
                      variant="ghost"
                      size="icon"
                      ...
                      onClick={() => {
                        setPastedImage(null);
                        form.setValue('discoveryAttachmentUrl', null);
                      }}
                    >
                      <X className="h-4 w-4" />
                    </Button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm font-medium text-slate-400 ml-9">
                      Select the category that best describes this observation.
                    </span>
                  </div>
                  <Controller
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-2">
                <div className="space-y-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm font-medium text-slate-400 ml-9">
                      Select the site where the observation occurred.
                    </span>
                  </div>
                  <Controller
                    control={form.control}
                ...
                <div className="space-y-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm font-medium text-slate-400 ml-9">
                      Provide the specific area, unit or equipment (if
                      applicable).
                    </span>
                  </div>
                  <div className="relative">
              ...
              <div className="pt-6 flex justify-end gap-3">
                <Button
                  type="button"
                  variant="outline"
                  disabled={form.formState.isSubmitting} onClick={() => {onOpenChange(false); form.reset(); setHandoff({assigneeId:"",targetDate:""}); setHandoffError("");}}
