...
export default function AddMobileDialog({ isOpen, setIsOpen }: AddMobileDialogProps) {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    toast({
      title: 'Item Added',
      description: `Mobile has been added.`,
    });
    setIsOpen(false);
    form.reset();
  };
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg" onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          ...
          <DialogDescription>Fill in the details for the new mobile device.</DialogDescription>
        </DialogHeader>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-2">
                ...
                <RadioGroup value={allotmentType} onValueChange={(value) => handleAllotmentTypeChange(value as 'user' | 'manpower')} className="flex gap-4">
                    <div className="flex items-center space-x-2"><RadioGroupItem value="user" id="user-mobile" /><Label htmlFor="user-mobile">App User</Label></div>
                    <div className="flex items-center space-x-2"><RadioGroupItem value="manpower" id="manpower-mobile" /><Label htmlFor="manpower-mobile">Manpower</Label></div>
                </RadioGroup>
            </div>
