...
import { CordlessReciprocatingSaw } from '@/lib/types';
import { Badge } from '../ui/badge';
import { ScrollArea } from '../ui/scroll-area';

interface CordlessReciprocatingSawTableProps {
  items: CordlessReciprocatingSaw[];
  onEdit: (item: CordlessReciprocatingSaw) => void;
}
...
export default function CordlessReciprocatingSawTable({ items, onEdit }: CordlessReciprocatingSawTableProps) {
  const { can, projects, deleteCordlessReciprocatingSaw } = useAppContext();
  const { toast } = useToast();

  const handleDelete = (itemId: string) => {
    deleteCordlessReciprocatingSaw(itemId);
    toast({
      variant: 'destructive',
      title: 'Item Deleted',
  ...
  if (items.length === 0) {
    return <p className="text-muted-foreground text-center py-8">No cordless reciprocating saws found.</p>;
  }
