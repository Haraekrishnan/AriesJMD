...
export default function NewManagementRequestDialog({ isOpen, setIsOpen }: NewManagementRequestDialogProps) {
  ...
  const possibleRecipients = users.filter(u => u.id !== user?.id && (u.role === 'Admin' || u.role === 'Manager' || u.role === 'Supervisor' || u.role === 'Project Coordinator' || u.role === 'Senior Safety Supervisor' || u.role === 'Document Controller'));
