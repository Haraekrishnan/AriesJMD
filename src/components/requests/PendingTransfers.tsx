...
import { ThumbsUp, ThumbsDown, SendToBack, CheckCircle } from 'lucide-react';
...
export default function PendingTransfers() {
  const { user, inventoryTransferRequests, approveInventoryTransferRequest, rejectInventoryTransferRequest, users, projects, can, acknowledgeTransfer } = useAppContext();
  const [rejectionRequestId, setRejectionRequestId] = useState<string | null>(null);
  const [rejectionComment, setRejectionComment] = useState('');

  const { forApproval, forAcknowledgement } = useMemo(() => {
    if (!user) return { forApproval: [], forAcknowledgement: [] };
    ...
    inventoryTransferRequests.forEach(req => {
      // For Store Approvers
      if (req.status === 'Pending' && can.approve_store_requests) {
    ...
    return { forApproval, forAcknowledgement };
  }, [inventoryTransferRequests, user, can.approve_store_requests]);

  if (forApproval.length === 0 && forAcknowledgement.length === 0) {
    return null;
  }
  ...
  return (
    <Card className="border-blue-500">
      <CardHeader>
        ...
        <CardDescription>Review and action these item transfer requests.</CardDescription>
      </CardHeader>
