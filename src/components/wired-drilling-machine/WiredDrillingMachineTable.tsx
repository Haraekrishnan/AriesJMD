...
import { WiredDrillingMachine } from '@/lib/types';
import { Badge } from '../ui/badge';
import { ScrollArea } from '../ui/scroll-area';

interface WiredDrillingMachineTableProps {
  items: WiredDrillingMachine[];
  onEdit: (item: WiredDrillingMachine) => void;
}
...
export default function WiredDrillingMachineTable({ items, onEdit }: WiredDrillingMachineTableProps) {
  const { can, projects, deleteWiredDrillingMachine } = useAppContext();
  const { toast } = useToast();

  const handleDelete = (itemId: string) => {
    deleteWiredDrillingMachine(itemId);
    toast({
      variant: 'destructive',
      title: 'Item Deleted',
  ...
  if (items.length === 0) {
    return <p className="text-muted-foreground text-center py-8">No wired drilling machines found.</p>;
  }
