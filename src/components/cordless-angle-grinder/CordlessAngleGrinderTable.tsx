...
import { CordlessAngleGrinder } from '@/lib/types';
import { Badge } from '../ui/badge';
import { ScrollArea } from '../ui/scroll-area';

interface CordlessAngleGrinderTableProps {
  items: CordlessAngleGrinder[];
  onEdit: (item: CordlessAngleGrinder) => void;
}
...
export default function CordlessAngleGrinderTable({ items, onEdit }: CordlessAngleGrinderTableProps) {
  const { can, projects, deleteCordlessAngleGrinder } = useAppContext();
  const { toast } = useToast();

  const handleDelete = (itemId: string) => {
    deleteCordlessAngleGrinder(itemId);
    toast({
      variant: 'destructive',
      title: 'Item Deleted',
  ...
  if (items.length === 0) {
    return <p className="text-muted-foreground text-center py-8">No cordless angle grinders found.</p>;
  }
