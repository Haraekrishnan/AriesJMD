...
const processItemsForMerging = (items: CertItem[]) => {
  ...
  items.forEach(item => {
    ...
    if (itemMap.has(key)) {
      ...
      existing.serialNumbers.push(item.manufacturerSrNo);
      existing.chestCrollNos.push(item.chestCrollNo);
      existing.ariesIds.push(item.ariesId);
    } else {
      itemMap.set(key, {
...
export async function generateTpCertExcel(
  ...
  const dateToUse = listDate
    ? typeof listDate === 'string'
      ? parseISO(listDate)
      : listDate
    : new Date();
