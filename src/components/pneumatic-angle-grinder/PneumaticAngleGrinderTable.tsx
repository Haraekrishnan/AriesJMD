...
import { PneumaticAngleGrinder } from '@/lib/types';
import { Badge } from '../ui/badge';
import { ScrollArea } from '../ui/scroll-area';

interface PneumaticAngleGrinderTableProps {
  items: PneumaticAngleGrinder[];
  onEdit: (item: PneumaticAngleGrinder) => void;
}
...
export default function PneumaticAngleGrinderTable({ items, onEdit }: PneumaticAngleGrinderTableProps) {
  const { can, projects, deletePneumaticAngleGrinder } = useAppContext();
  const { toast } = useToast();

  const handleDelete = (itemId: string) => {
    deletePneumaticAngleGrinder(itemId);
    toast({
      variant: 'destructive',
      title: 'Item Deleted',
  ...
  if (items.length === 0) {
    return <p className="text-muted-foreground text-center py-8">No pneumatic angle grinders found.</p>;
  }
