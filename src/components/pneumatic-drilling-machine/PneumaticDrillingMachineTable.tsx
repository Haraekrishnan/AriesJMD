...
import { PneumaticDrillingMachine } from '@/lib/types';
import { Badge } from '../ui/badge';
import { ScrollArea } from '../ui/scroll-area';

interface PneumaticDrillingMachineTableProps {
  items: PneumaticDrillingMachine[];
  onEdit: (item: PneumaticDrillingMachine) => void;
}
...
export default function PneumaticDrillingMachineTable({ items, onEdit }: PneumaticDrillingMachineTableProps) {
  ...
  const handleDelete = (itemId: string) => {
    deletePneumaticDrillingMachine(itemId);
    toast({
      variant: 'destructive',
      title: 'Item Deleted',
  ...
  if (items.length === 0) {
    return <p className="text-muted-foreground text-center py-8">No pneumatic drilling machines found.</p>;
  }
