...
type AppContextType = {
  ...
  updateLogbookRequestStatus: (requestId: string, status: 'Completed' | 'Rejected') => void;
};
...
const markSinglePlannerCommentAsRead = useCallback((plannerUserId: string, day: string, commentId: string) => {
  ...
  const addDocument = useCallback((data: Omit<DownloadableDocument, 'id' | 'uploadedBy' | 'createdAt'>) => {
    ...
    set(newRef, newDoc);
    addActivityLog(user.id, 'Document Added', data.title);
  }, [user, addActivityLog]);
  ...
  const updateLogbookRequestStatus = useCallback((requestId: string, status: 'Completed' | 'Rejected') => {
